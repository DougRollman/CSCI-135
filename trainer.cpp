...
Trainer::Trainer(std::vector<Profemon> profemons){
    for (const Profemon& profemon : profemons) {
        if (!addProfemon(profemon)) {
            break;  // Stop adding if the team is already full
        }
    }
    chooseProfemon(0);  // Select the first profémon in the team
}
/*
...
bool Trainer::contains(std::string name) {
    // Check if the name exists in the team
    for (int i = 0; i < 3; i++) {
    ...
    // Check if the name exists in the profédex
    ...
    // Name not found in team or profédex
    return false;
}
/*
...
bool Trainer::addProfemon(Profemon profemon) {
    // Check if the profémon already exists in the team or profédex
    if (contains(profemon.getName())) {
        return false;
    }

    // Check if there is an empty slot in the team
    for (int i = 0; i < 3; i++) {
    ...
    // Add the profémon to the profédex if the team is full
...
bool Trainer::removeProfemon(std::string name) {
    // Check if the profémon exists in the team
    for (int i = 0; i < 3; i++) {
        if (team[i].getName() == name) {
            team[i] = Profemon();  // Replace with an empty profémon
            return true;
        }
    }

    // Check if the profémon exists in the profedex
    // received help for this portion of the code
    for (int i = 0; i < profedex.size(); i++) {
        if (profedex[i].getName() == name) {
            profedex.erase(profedex.begin() + i);  // Remove the profémon from the profedex
            return true;
        }
    }

    return false;  // Profémon not found
}
/*
...
void Trainer::setTeamMember(int slot, std::string name) {
    // Check if the profémon exists in the profédex
    for (int i = 0; i < profedex.size(); i++) {
        // If the profémon's name matches the given name
        if (profedex[i].getName() == name) {
            // Check if there is already a profémon in the specified slot
            if (team[slot].getName() != "Undefined") {
                // Swap the profémon in the slot with the one found in the profédex
                std::swap(team[slot], profedex[i]);
            }
            else {
                // Assign the found profémon to the specified slot in the team
                team[slot] = profedex[i];
                // Remove the profémon from the profédex
                profedex.erase(profedex.begin() + i);
            }
            // Exit the function since the operation is complete
            return;
        }
    }
}
...
void Trainer::printProfedex(){
    for (int i = 0; i < this->profedex.size(); i++) {
        if (this->profedex[i].getName() != "Undefined") {
            this->profedex[i].printProfemon(false);
            std::cout << std::endl;
        }
    }
}
